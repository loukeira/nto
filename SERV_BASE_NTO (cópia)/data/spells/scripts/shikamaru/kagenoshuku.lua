...
local effect = {216} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
if (isCreature(cid)) then
    if getPlayerStorageValue(cid, 102059) > 0 and getCreatureCondition(cid, CONDITION_REGENERATION, 1) then
        for i=1, #effect do
...
if getPlayerStorageValue(cid, 102053) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    setPlayerStorageValue(cid, 102059,1) -- storage verifica transformado, quando = 1 player esta transformado.
else
