...
local effect = {118} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
function magicEffect555252581(tempo2,tempo3,cid)
...
if getPlayerStorageValue(cid, 102053) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    while (tempo2 ~= (tempo*1000)) do
        addEvent(magicEffect555252581, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 300
    end
    setPlayerStorageValue(cid, 102053,1) -- storage verifica transformado, quando = 1 player esta transformado.
    doCreatureSay(cid, "Susanoo Mangekyou", TALKTYPE_MONSTER)
