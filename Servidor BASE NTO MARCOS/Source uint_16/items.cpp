...
int32_t Items::loadFromOtb(std::string file)
{
	...
	if(Items::dwMajorVersion == 0xFFFFFFFF)
		std::clog << "[Atencao - Items::loadFromOtb] items.otb usando a versão genérica do cliente." << std::endl;
	else if(Items::dwMajorVersion < 3)
	{
		std::clog << "[Erro - Items::loadFromOtb] Versão antiga detectada , Uma versão mais recente de items.otb é necessária." << std::endl;
		return ERROR_INVALID_FORMAT;
	}
	else if(Items::dwMajorVersion > 3)
	{
		std::clog << "[Erro - Items::loadFromOtb] Nova versão detectada, é necessária uma versão mais antiga do items.otb." << std::endl;
		return ERROR_INVALID_FORMAT;
	}
	else if(!g_config.getBool(ConfigManager::SKIP_ITEMS_VERSION) && Items::dwMinorVersion != CLIENT_VERSION_860)
	{
		std::clog << "[Erro - Items::loadFromOtb] É necessária outra versão (de cliente) de items.otb." << std::endl;
		return ERROR_INVALID_FORMAT;
	}
...
bool Items::loadFromXml()
{
	...
	if(!itemDoc)
	{
		std::clog << "[Atenção - Items::loadFromXml] Não é possível carregar o arquivo de itens." << std::endl;
	...
	if(!paletteDoc)
	{
		std::clog << "[Atenção - Items::loadFromXml] Não é possível carregar o arquivo de aleatorização." << std::endl;
		std::clog << getLastXMLError() << std::endl;
		return false;
	}
	...
	if(xmlStrcmp(itemRoot->name,(const xmlChar*)"items"))
	{
		...
		std::clog << "[Atenção - Items::loadFromXml] Arquivo de itens malformados." << std::endl;
		return false;
	}
	...
	if(xmlStrcmp(paletteRoot->name,(const xmlChar*)"randomization"))
	{
		...
		std::clog << "[Atenção - Items::loadFromXml] Arquivo de aleatorização malformado." << std::endl;
		return false;
	}
	...
	for(xmlNodePtr itemNode = itemRoot->children; itemNode; itemNode = itemNode->next)
	{
		...
		else if(readXMLString(itemNode, "fromid", strValue) && readXMLString(itemNode, "toid", endValue))
		{
			...
			if(intVector[0] && endVector[0] && intVector.size() == endVector.size())
			{
				...
				std::clog << "[Atenção - Items::loadFromXml] Entrada malformada (a partir de: \"" << strValue << "\", para: \"" << endValue << "\")" << std::endl;
		}
		else
			std::clog << "[Atenção - Items::loadFromXml] Nenhum itemid encontrado" << std::endl;
	}
	...
	for(uint32_t i = 0; i < Item::items.size(); ++i) //lets do some checks...
	{
		...
		if((it->transformToFree || it->transformUseTo[PLAYERSEX_FEMALE] || it->transformUseTo[PLAYERSEX_MALE]) && it->type != ITEM_TYPE_BED)
			std::clog << "[Atenção - Items::loadFromXml] Item " << it->id << " Não está configurado como um tipo de cama." << std::endl;
	}
	...
	for(xmlNodePtr paletteNode = paletteRoot->children; paletteNode; paletteNode = paletteNode->next)
	{
		if(!xmlStrcmp(paletteNode->name, (const xmlChar*)"config"))
		{
			if(readXMLInteger(paletteNode, "chance", intValue) || readXMLInteger(paletteNode, "defaultChance", intValue))
			{
				...
				{
					intValue = 100;
					std::clog << "[Atenção - Items::loadFromXml] A chance aleatória não pode ser maior do que 100." << std::endl;
				}
		...
		else if(!xmlStrcmp(paletteNode->name, (const xmlChar*)"palette"))
		{
			...
			if(itemList.size() > 1)
			{
				if(itemList[0] < itemList[1])
				{
					...
					std::clog << "[Atenção - Items::loadFromXml] Randomize min não pode ser maior do que max." << std::endl;
			}
			...
			if(readXMLInteger(paletteNode, "chance", intValue))
			{
				...
				{
					intValue = 100;
					std::clog << "[Atenção: Items::loadRandomization] A chance aleatória não pode ser maior do que 100." << std::endl;
				}
...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	if(!it.name.empty() && (!readXMLString(itemNode, "override", strValue) || !booleanString(strValue)))
		std::clog << "[Atenção - Items::loadFromXml] item duplicado, registrado com id " << id << std::endl;
