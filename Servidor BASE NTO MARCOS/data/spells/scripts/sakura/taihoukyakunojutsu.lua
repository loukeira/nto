...
if target > 0 then
	...
	if position_target ~= nil then
		...
		while (tempo ~= quant_hit*(timeHit*1000)) do
			addEvent(sendEffect2, tempo, cid, target)
			tempo = tempo + 500
		end
		tempo = 0
		while (tempo ~= quant_hit*(timeHit*1000)) do
			addEvent(hitTarget2, tempo, cid, target)
...
local hit2 = getPlayerLevel(cid)*2 + getPlayerMagLevel(cid)*2 + math.random(28650,29750)
return hit2
end
