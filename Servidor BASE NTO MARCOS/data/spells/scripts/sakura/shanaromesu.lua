...
efeitoDamage = 185, -- efeito q ira aparecer ao hitar no alvo
hits = 3, -- quantos hits vai dar
delay = 400, -- intervalo de tempo a cada hit
min = 7300, -- dano minimo
max = 8200, -- dano maximo
...
spell9 = {
start9 = function (cid, target, markpos, hits)
    ...
    addEvent(spell9.start9, config.delay, cid, target, markpos, hits - 1)
   end
}
...
if target then
  spell9.start9(cid, target, getThingPos(cid), config.hits)
doSendMagicEffect(position2, 22)
end
return true
end