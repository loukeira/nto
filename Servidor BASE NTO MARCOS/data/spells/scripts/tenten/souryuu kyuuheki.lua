...
local effect = {126} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
function magicEffect34(tempo2,tempo3,cid)
...
doPlayerSendTextMessage(cid,27,'jutsu esta ativado.') 
    doPlayerSay(cid, '...', TALKTYPE_ORANGE_1)
    tempo2 = 0
    while (tempo2 ~= (tempo*1000)) do
        addEvent(magicEffect34, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 300
    end
    setPlayerStorageValue(cid, 102053,1) -- storage verifica transformado, quando = 1 player esta transformado.
        doCreatureSay(cid, "Souryuu Kyuuheki", TALKTYPE_MONSTER)
else
