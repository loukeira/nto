...
doBroadcastMessage("Noticia: Jornal Shinobi informa: NPC OININ, informou que o shinobi "..msg.." esta procurado, e agora o seu nivel de recompensa e procura é "..getPlayerStorageValue(getPlayerByNameWildcard(msg), 13131)..".", BroadCast_Type)
else
selfSay('Descupe, voce não possui o dinheiro necessário para por uma recompensa a cabeça a este jogador.', cid)
end
else
...
selfSay("O rank de recompensa deste shinobi é "..getPlayerStorageValue(getPlayerByNameWildcard(msg), 13131)..".", cid)
else
...
selfSay("O seu rank de recompensa é "..getPlayerStorageValue(cid, 13131)..".", cid)
else
...
selfSay('O sistema funciona da seguinte maneira: voce pode colocar recompensas pela cabeça de jogadores, assim, quem matar estes jogadores irá ganhar um bom dinheiro, porem, ha um alto custo para utilizar os meus serviços.', cid)
talkState[talkUser] = 0
end
