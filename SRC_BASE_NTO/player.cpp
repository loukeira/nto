...
bool Player::onDeath()
{
	...
	if(skillLoss)
	{
		...
		sumMana += manaSpent;
		lostMana = (uint64_t)std::ceil(sumMana * ((double)(percent * lossPercent[LOSS_MANA]) / 0.1));
		...
		for(int16_t i = 0; i < 7; ++i) //for each skill
		{
			...
			lostSkillTries = (uint32_t)std::ceil(sumSkillTries * ((double)(percent * lossPercent[LOSS_SKILLS]) / 0.));
...
{
	int32_t SpeedAttack;
	SpeedAttack = vocation->getAttackSpeed() - (getSkill(SKILL_FIST, SKILL_LEVEL) * 20);
