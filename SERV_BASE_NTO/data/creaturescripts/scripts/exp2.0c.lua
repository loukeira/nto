...
multiplier = multiplier - 0.25
	...
	local nulo = (isInArray(effectiveness[damageCombat].non, poketype1) and isInArray(effectiveness[damageCombat].non, poketype2))
...
if damageCombat ~= COMBAT_PHYSICALDAMAGE and not isInArray(ignored, damageCombat) then
   if isInArray(effectiveness[damageCombat].non, poketype1) or isInArray(effectiveness[damageCombat].non, poketype2) then
   if isSummon(attacker) and isPlayer(getCreatureMaster(attacker)) then
      if not isInArray(specialabilities["foresight"], getCreatureName(attacker)) and not getItemAttribute(getPlayerSlotItem(getCreatureMaster(attacker), 8).uid, "attackheld") == 1 then   
         valor = valor * 0   
      end
	  else 
	  valor = valor * 0  
...
------------------------- X-REGENERATION -----------------------------
if isSummon(attacker) and isSummon(cid) and isPlayer(getCreatureMaster(cid)) and getPlayerStorageValue(getCreatureMaster(cid), 637501) <= 0 then
