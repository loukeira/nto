...
local quant_sqm = 7 -- sqms que ira empurrar o target
...
setConditionParam(condition, CONDITION_PARAM_SPEED, -450)
...
if target > 0 then
	...
	if position_target ~= nil then
		...
		while (tempo ~= quant_hit*(timeHit*1000)) do
			addEvent(sendEffect423, tempo, cid, target)
			tempo = tempo + 500
		end
		tempo = 0
		while (tempo ~= quant_hit*(timeHit*1000)) do
...
if isCreature(target) and isPlayer(cid) then
	local effect423 = 1 -- efeito do hit na creatura.
	doAddCondition(target, condition)
