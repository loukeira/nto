...
local ml = 100 -- quantos ira aumentar o skill de ML
...
local skilldistance = 100 -- quantos ira aumentar o skill de Distance
...
function magicEffect51(tempo2,tempo3,cid)
if (isCreature(cid)) then
    if getCreatureCondition(cid, CONDITION_REGENERATION, 1) then

            doSendMagicEffect(getPlayerPosition(cid), 154)
        end
    end
end
...
if getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    while (tempo2 ~= (tempo*1000)) do
        addEvent(magicEffect51, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 1000
    end
end
end