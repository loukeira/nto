local storage_unica_dessa_spell = ninposozosaisei1 -- MUDAR ESSA STORAGE.. CUIDADO COM A SEQUENCIA...
local storage_unica_dessa_spell_2 = ninposozosaisei2 --MUDAR ESSA STORAGE... CUIDADO COM A SEQUENCIA..
...
local effect = 321 -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
--mudar aqui \/        
function magicEffect011(tempo2, tempo_de_intervalo_da_effect ,effect,cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)

                        if (isCreature(cid)) then
                        local stor = getPlayerStorageValue(cid,storage_unica_dessa_spell_2)
                        if stor == -1  then
--mudar aqui \/        
addEvent(pararmagicEffect011, tempo2*1000 ,cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
...
--mudar aqui \/        
addEvent(magicEffect011, tempo_de_intervalo_da_effect ,0,tempo_de_intervalo_da_effect, effect, cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
                    ...
                    if (getPlayerStorageValue(cid, storage_unica_dessa_spell) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false) and (get_buff_on(cid) < 1) then
                        ...
                        tempo2 = tempo-1
--mudar aqui \/        
addEvent(magicEffect011, 1000, tempo2, tempo_de_intervalo_da_effect, effect, cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
...
--mudar aqui \/        
function pararmagicEffect011( cid,storage_unica_dessa_spell,storage_unica_dessa_spell_2 )

--mudar aqui \/        
stopEvent(magicEffect011)
