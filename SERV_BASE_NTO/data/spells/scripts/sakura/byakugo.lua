local storage_unica_dessa_spell = byakugo1 -- MUDAR ESSA STORAGE.. CUIDADO COM A SEQUENCIA...
local storage_unica_dessa_spell_2 = byakugo2 --MUDAR ESSA STORAGE... CUIDADO COM A SEQUENCIA..
...
local health = 300 -- A cada 1 segundo quantos aumentar de vida
                        ...
                        setConditionParam(condition, CONDITION_PARAM_HEALTHTICKS, 300)
                        setCombatCondition(combat, condition)
						
            
--mudar aqui \/        
function magicEffect010(tempo2, tempo_de_intervalo_da_effect ,effect,cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)

                        if (isCreature(cid)) then
                        local stor = getPlayerStorageValue(cid,storage_unica_dessa_spell_2)
                        if stor == -1  then
--mudar aqui \/        
addEvent(pararmagicEffect010, tempo2*1000 ,cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
...
--mudar aqui \/        
addEvent(magicEffect010, tempo_de_intervalo_da_effect ,0,tempo_de_intervalo_da_effect, effect, cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
                    ...
                    if (getPlayerStorageValue(cid, storage_unica_dessa_spell) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false) and (get_buff_on(cid) < 1) then
                        ...
                        tempo2 = tempo-1
--mudar aqui \/        
addEvent(magicEffect010, 1000, tempo2, tempo_de_intervalo_da_effect, effect, cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
...
--mudar aqui \/        
function pararmagicEffect010( cid,storage_unica_dessa_spell,storage_unica_dessa_spell_2 )

--mudar aqui \/        
stopEvent(magicEffect010)
