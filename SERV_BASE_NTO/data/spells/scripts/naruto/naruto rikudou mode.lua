local storage_unica_dessa_spell = narutorikudo1 -- MUDAR ESSA STORAGE.. CUIDADO COM A SEQUENCIA...
local storage_unica_dessa_spell_2 = narutorikudo2 --MUDAR ESSA STORAGE... CUIDADO COM A SEQUENCIA..
...
--mudar aqui \/        
function magicEffect006(tempo2, tempo_de_intervalo_da_effect ,effect,cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)

                        if (isCreature(cid)) then
                        local stor = getPlayerStorageValue(cid,storage_unica_dessa_spell_2)
                        if stor == -1  then
--mudar aqui \/        
addEvent(pararmagicEffect006, tempo2*1000 ,cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
...
--mudar aqui \/        
addEvent(magicEffect006, tempo_de_intervalo_da_effect ,0,tempo_de_intervalo_da_effect, effect, cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
                    ...
                    if (getPlayerStorageValue(cid, storage_unica_dessa_spell) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false) and (get_buff_on(cid) < 1)then
                        ...
                        tempo2 = tempo-1
--mudar aqui \/        
addEvent(magicEffect006, 1000, tempo2, tempo_de_intervalo_da_effect, effect, cid, storage_unica_dessa_spell, storage_unica_dessa_spell_2)
