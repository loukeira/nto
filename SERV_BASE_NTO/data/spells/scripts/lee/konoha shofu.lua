...
spell8 = {
start8 = function (cid, target, markpos, hits)
    ...
    addEvent(spell8.start8, config.delay, cid, target, markpos, hits - 1)
   end
}
...
if target then
  spell8.start8(cid, target, getThingPos(cid), config.hits)
doSendMagicEffect(position1, 64)
doSendMagicEffect(position2, 180)
doSendMagicEffect(position3, 212)
end
return true
end